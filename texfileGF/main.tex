...
\usepackage{hyperref}
\usepackage{bm,physics,mathrsfs,enumitem}
\usepackage{fancyhdr}
%footnote
\pagestyle{fancy}
...
\textbf{Giải}
%\begin{enumerate}[label=(\alph*)]
%	\item Ta có hàm Green 
%	\begin{align}
%		G(x,t) = \left[ \Theta(t - x)x(1-t) + \Theta(x - t)t(1-x) \right],
%	\end{align}
%	bên cạnh đó ta đã chứng minh được rằng hàm Green là toán tử tự phó thì có dạng
%	\begin{align}
%		G(x,t) = \sum_{n = 1}^{\infty} \f{1}{\lambda_n} Y_{n}(x) Y_{n}^{*}(t)
%	\end{align}
%	trong đó $\lambda_n$ là trị riêng của toán tử Sturm-Liouville. Ta khai triển phương trình trên theo phương trình (6.36. /D.Skinner). 
%	\begin{align}
%		\delta(x - t) = w(x) \sum_{n \in \mathbb{Z}}^{\infty} Y_{n}(x) Y_{n}^{*}(t).
%	\end{align}
%	Xem $t$ như là tham số, ta dẫn ra được khai triển hàm riêng của hàm Green là
%	\begin{align}
%		G(x,t) = \sum_{n \in \mathbb{Z}} \hat{G}_n(t) Y_n(x).
%	\end{align}
%	Áp dụng toán tử tự phó $\mathcal{L}$ ta có
%	\begin{align}
%		\mathcal{L} G = \sum_{n \in \mathbb{Z}} \hat{G}_n(t) \mathcal{L} Y_n(x) = w(x) \sum_{n \in \mathbb{Z}} \hat{G}_n(t) \lambda_n Y_n(x),
%	\end{align}
%	để điều này là một hàm $\delta(x - t)$ thì khai triển trên phải là thỏa định nghĩa $\mathcal{L} G = \delta(x-t)$ cho hàm Green, ta cần phải
%	\begin{align}
%		\hat{G}_n(t) = \f{1}{\lambda_n} Y_n^{*}(x).
%	\end{align}
%	
%	Vì hàm Green là nghiệm duy nhất của phương trình $\mathcal{L} G(x,t) =\delta (x-t)$ thỏa điều kiện $G(a,t) = G(b,t) = 0$, vì thế khai triển (3.17) và (3.18) là giống nhau. Để kiểm chứng điều này, ta nhận thấy rằng $\mathcal{L}$ ở dạng
%	\begin{align*}
%		\mathcal{L}  = \alpha(x) \f{\partial^2}{\partial x^2} + \beta(x) \f{\partial}{\partial x} + \gamma(x)
%	\end{align*}
%	hai hệ số đầu tiên của phương trình trên có mối liên hệ là $\beta = d\alpha/dx $. Vì thế trong trường hợp này, thành phần $\alpha(t) W(t)$	 trong phương trình phải là hằng số khác \textit{không}. Để kiểm chứng điều này ta phải làm
%	\begin{align*}
%		\f{d}{dx}(aW) 
%		& = a' W + a W = \beta (y_1 y'_2 - y'_1 y_2) + \alpha (y_1 y''_2 - y''_1 y_2) \\
%		& =	0,	
%	\end{align*}
%	vậy nên $a(t)W(t)$ là hằng số, ta có thể viết lại phương trình (3.17) thành dạng
%	\begin{align*}
%		G(x,t) = \f{1}{c} \left[ \Theta(t - x)x(1-t) + \Theta(x - t)t(1-x) \right].
%	\end{align*}	
%\end{enumerate}
